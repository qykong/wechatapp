...
Page({
  ...
  onLoad: function (options) {
    ...
    navTitle = '';
    if (parent_tid != 0) {
      navTitle = options.tname + '-' + options.parent_tname + "信息列表";
    } else {
      navTitle = options.tname + " 信息列表";
    }
    ...
    //加载幻灯片  
    setTimeout(function () {
      that.load_banners(options.tid);
    }, 300);

    setTimeout(function () {
      that.load_mtype_parent(options.tid);
    }, 310);
  ...
  //加载分类的子类型
  load_mtype_parent: function (_typeid) {
    var that = this;
    app.util.request({
      'url': 'entry/wxapp/get_mtype_list_byid',
      data: { typeid: _typeid },
      success(res) {
        var json = res.data.data;
        if (json.length > 0) {
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      title: app.globalData.share_name + " " + navTitle,
      path: '/' + app.globalData.wx_model_name +'/pages/msg_list/msg_list?tid=' + tid + "&tname=" + tname + '&parent_tid=' + parent_tid + '&parent_tname=' + parent_tname,
  ...
  load_list: function (m_type, _page) {
    ...
    app.util.request({
      ...
      data: {
        ...
        m_parent_typeid: that.data.parent_tid,
        openid: app.globalData.openId,
  ...
  tap_add: function (e) {
    ...
    var mtype_totype = that.data.mtype_totype;
    if (mtype_totype == '1') {
      that.util_model('open', 'menu');
    } else if (mtype_totype == '0') {
      wx.redirectTo({
        url: '../msg_send/msg_send?tid=' + tg.tid + '&tname=' + tg.tname + '&parent_tid=' + 0 + '&parent_tname='
      });
  ...
  tap_add_sheet: function (e) {
    ...
    wx.redirectTo({
      url: '../msg_send/msg_send?tid=' + tg.tid + '&tname=' + tg.tname + '&parent_tid=' + tg.pid + '&parent_tname=' + tg.pname
    });
